...
# プロフィール記事 生成・更新指示書（Copilot用）

この指示書は、AI（GitHub Copilot等）に「プロフィール」セクションの記事を追加・更新させる際の標準手順です。

---

## 1. 記事内容の準備
- タイトル、説明、本文、作成日などを用意する
- 記事ファイルのパス例：`docs/profiles/` 配下

## 2. 記事ファイルの基本構成と各要素の記述ルール

### 2-1. Docusaurusメタデータ（YAMLフロントマター）
- 記事の一番最初には、必ず Docusaurus メタデータ（YAMLフロントマター）を記載してください。
- メタデータのテンプレートは `docs/profiles/.template.md` など既存記事を参照してください。
...
- 新しい記事を追加・カテゴリを編集した場合、必ず `src/data/ateliers-dev_content-list.json`（テーブル用）と `sidebars.ts`（サイドバー用）のカテゴリ名・順序・階層を一致させてください。
	- 例：テーブルで「プロフィール」カテゴリに記事を追加した場合、サイドバーの同名カテゴリにも同じ順序・内容で追加すること。
	- 必ず `"id": "profiles"` の配下（profilesカテゴリ内）に配置してください。
...
- `sidebars.ts` の `profileSidebar` も同じカテゴリ・順序・階層で記事IDを追加してください。
- `docs/profiles/index.md` の一覧にもリンクを追加

---

この指示書に従い、プロフィール記事の追加・更新をAIに依頼してください。
